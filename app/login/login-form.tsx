"use client"; // <--- BẮT BUỘC: Đánh dấu Client Component

import { useState } from "react";
import { Lock, Mail, Eye, EyeOff } from "lucide-react"; // Import thêm Eye, EyeOff
        ...
        <div className="space-y-2">
          ...
          <div className="relative">
            <input
              id="password"
              name="password"
              // Logic: Nếu showPassword = true thì type="text", ngược lại "password"
              type={showPassword ? "text" : "password"}
              placeholder="••••••••"
              required
              // pr-10 để chữ không bị đè lên icon mắt
            ...
            >
              {showPassword ? (
                <EyeOff className="h-5 w-5" /> // Mắt gạch chéo
              ) : (
                <Eye className="h-5 w-5" /> // Mắt mở
              )}
            </button>
          </div>
        </div>
