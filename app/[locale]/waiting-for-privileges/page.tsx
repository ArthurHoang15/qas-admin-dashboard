...
export default function WaitingForPrivilegesPage() {
  const t = useTranslations("auth");
  const router = useRouter();
  const [checking, setChecking] = useState(false);

  // Auto-refresh every 30 seconds to check if permissions have been assigned
  useEffect(() => {
      ...
      <div className="w-full max-w-md bg-white rounded-2xl border border-slate-100 shadow-xl overflow-hidden">
        <div className="p-8 text-center">
          ...
          <h1 className="text-2xl font-bold text-slate-900 tracking-tight mb-3">
            {t("waitingForPrivileges.title")}
          </h1>

          <p className="text-slate-600 mb-8">
            {t("waitingForPrivileges.description")}
          </p>
          ...
          <div className="flex flex-col gap-3">
            ...
            >
              ...
              {t("waitingForPrivileges.checkNow")}
            </button>

            <div className="p-4 bg-slate-50 rounded-lg border border-slate-100">
              <p className="text-sm text-slate-500">
                {t("waitingForPrivileges.autoRefresh")}
              </p>
            </div>
